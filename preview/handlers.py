            ...
            while True:
                ...
                try:
                    ...
                    if view_proc.returncode != 0:
                        alert(
                            text="There was a problem during the view phase.\nCommand Output:\n"
...
class JustHandler(BaseHandler):
    ...
    async def build(self):
        ...
        if proc.returncode != 0:
            alert(
                text="There was a problem during build phase.\nCommand Output:\n"
                + output[0].decode("utf-8")
                + "\n"
            )
...
def find_a_handler(input_file: pathlib.Path):
    ...
    raise RuntimeError(
        f"Could not find a handler for filtype `{filetype}` to preview `{input_file}`."
    )
...
def get_example_handler_filenames():
    ...
    filenames += map(
        ...
        itertools.product(["justfile", "makefile"], ["md", "tex", "gnuplot"]),
    )
